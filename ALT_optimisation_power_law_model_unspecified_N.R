##############################################################################
### Test plan optimisation for Weibull power law AFT model (Unspecified N) ###
##############################################################################
...
#The optimisation algorithm returns the optimal test plan, i.e., the optimal 
#number of stresses, the stress levels and the associated number of test units.
...
### Function 1. Weibull log likelihood function (power law AFT model) ###
...
objective_function <- function(x, n_sim = 1000, test_units = 100, 
                               test_duration = 1*24*365,  min_units = 1, 
                               stress_distance = 10^(-5), max_stress = 0.9, 
                               max_stress_levels = 6, design_stress = 0.05,
                               p_cens_limit = 0.8, filter_limit = 0.95,  
                               pquant = 0.5, model_params = c(-7, -6, 2), 
                               dp = 5, s1LB = 0.1, s1UB = 0.9 - 10^(-5), 
                               dLB = 10^(-5), dUB = 0.8) 
{
  ...
  lambda <- exp(B0 + B1*log(stress))
  true_quantile <- exp(B0 + B1*log(design_stress))*(-log(1 - pquant))^(1/gamma)
  ...
  while (i <= n_sim) 
  {
    ...
    for (j in 1:length(init_values)) 
    {
      ...
      if (!inherits(result, "try-error") && is.finite(result$minimum)) 
      {
        ...
        {
          est_quantile[i] <- exp(mle_b0 + mle_b1*log(design_stress))*
            (-log(1 - pquant))^(1/mle_gamma)
          break
        }
      }
    }
...
ALT_optimise <- function(n_sim = 1000, test_units = 100, 
                         test_duration = 1*24*365, min_units = 1, 
                         min_stress_levels = 2, max_stress_levels = 6, 
                         stress_distance = 10^(-5), min_stress = 0.1, 
                         max_stress = 0.9, design_stress = 0.05,   
                         p_cens_limit = 0.8, filter_limit = 0.95,  
                         pquant = 0.5, model_params = c(-7, -6, 2), dp = 5,
                         DE_iter = 200, DE_step = 200, DE_tol = 0.05)
{
  ...
  DELB <- c(min_stress_levels, s1LB, rep(dLB, times = (max_stress_levels - 1)), 
            rep(-5, times = max_stress_levels))
  DEUB <- c(max_stress_levels, s1UB, rep(dUB, times = (max_stress_levels - 1)), 
            rep(5, times = max_stress_levels))
  ...
  DEoptim_results <- DEoptim(objective_function, lower = DELB, upper = DEUB, 
                             ...
                             n_sim = n_sim, test_units = test_units, 
                             max_stress = max_stress, min_units = min_units,
                             stress_distance = stress_distance,  
                             p_cens_limit = p_cens_limit, 
                             filter_limit = filter_limit,
                             design_stress = design_stress, 
                             test_duration = test_duration, pquant = pquant, 
                             model_params = model_params, dp = dp, 
                             max_stress_levels = max_stress_levels,
...
### 1. Initial model parameters (b0, b1, gamma) ###
...
n_sim <- 1000 
#Minimum number of stress levels (design points) (must be >= 2)
min_stress_levels <- 2
#Maximum number of stress levels (design points)
max_stress_levels <- 6
...
#The optimisation settings are chosen to demonstrate the optimisation process/output.
...
demo_optimisation_results <- ALT_optimise(model_params = model_params, 
                                          ...
                                          design_stress = design_stress,
                                          min_stress_levels = min_stress_levels,
                                          max_stress_levels = max_stress_levels,
                                          DE_tol = DE_tol,
...
#The following optimisation settings may be suitable for initial/exploratory analysis.
#Execution time ~ hour

set.seed(202508)
initial_optimisation_results <- ALT_optimise(model_params = model_params, 
                                             test_units = test_units, 
                                             test_duration = test_duration, 
                                             min_units = min_units,
                                             p_cens_limit = p_cens_limit, 
