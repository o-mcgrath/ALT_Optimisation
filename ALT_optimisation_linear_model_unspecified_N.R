###########################################################################
### Test plan optimisation for Weibull linear AFT model (Unspecified N) ###
###########################################################################
#The user specifies initial model parameters (following a preliminary experiment and/or SME input)
#The user specifies a range of stress levels to consider
#The algorithm returns the optimal number of stresses, the optimal stress levels and the associated number of test units
...
### Function 1. Weibull log likelihood function (linear model) ###
...
objective_fnc <- function(x, n_sim = 1000, test_units = 100, test_duration = 1*24*365,  min_units = 1, 
                          stress_distance = 10^(-5), max_stress = 0.9, max_stress_levels = 6, design_stress = 0.05,
                          p_cens_limit = 0.8, filter_limit = 0.95,  
                          pquant = 0.5, model_params = c(13, -19, 2), dp = 5,
                          s1LB = 0.1, s1UB = 0.9 - 10^(-5), dLB = 10^(-5), dUB = 0.8) 
{
  design_pts <- as.integer(round(x[1]))
  s1_UB_N <- max_stress - (stress_distance*(design_pts - 1))
  p_s1 <- (x[2] - s1LB)/(s1UB - s1LB) 
  scaled_s1 <- p_s1*(s1_UB_N - s1LB) + s1LB
  d_UB_N <- (s1_UB_N + stress_distance) - min_stress 
  p_d <- (x[3:(design_pts + 1)] - dLB)/(dUB - dLB)
  scaled_d <- p_d*(d_UB_N - dLB) + dLB
  
  first_zi <- (max_stress_levels + 2) 
  zi <- x[first_zi: (first_zi + (design_pts - 1))]
  new_x <- c(scaled_s1, scaled_d, zi)
    
  s1_d <- round(new_x[1:design_pts], dp) 
  ...
  zi <- new_x[(design_pts + 1):(2*design_pts)]
  p <- exp(zi)/sum(exp(zi))
...
results_fnc <- function(x, test_units = 100, min_units = 1, stress_distance = 10^(-5), max_stress = 0.9,  max_stress_levels = 6, dp = 5, s1LB = 0.1, 
                        s1UB = 0.9 - 10^(-5), dLB = 10^(-5), dUB = 0.8, rmse) 
{
  design_pts <- as.integer(round(x[1]))
  s1_UB_N <- max_stress - (stress_distance*(design_pts - 1))
  p_s1 <- (x[2] - s1LB)/(s1UB - s1LB) 
  ...
  names(optimal_values) <- c("N", paste("s", 1:design_pts, sep = ""), paste("n", 1:design_pts, sep = ""), "DEoptim_RMSE")
...
experiment_config_fnc <- function(n_sim = 1000, test_units = 100, test_duration = 1*24*365, min_units = 1, 
                                  min_stress_levels = 2, max_stress_levels = 6, stress_distance = 10^(-5), max_stress = 0.9, min_stress = 0.1, design_stress = 0.05,   
                                  p_cens_limit = 0.8, filter_limit = 0.95,  
                                  ...
                                  DE_iter = 200, DE_step = 200, DE_tol = 0.05)
{
  config_check <- c(
    ...
    check15 = min_stress_levels >= 2,
    check16 = min_stress_levels <= max_stress_levels,
    check17 = all(c(max_stress, stress_distance) > 0),
    check18 = min_units <= test_units/max_stress_levels,
    check19 = stress_distance*max_stress_levels <= max_stress,
    check20 = all(c(DE_iter, DE_step, DE_tol) > 0))
  ...
  error_message <- c(
    ...
    check16 = "The minimum stress levels cannot be greater than the maximum stress levels.",
    check17 = "The maximum stress and stress distance must be greater than zero.",
    check18 = "The desired number of test units for the specified stress levels exceeds the total test units.",
    check19 = "(maximum number of stress levels * distance between each stress) exceeds the maximum stress.",
    check20 = "DEoptim control/stopping criteria values must be greater than zero.")
...
DEoptim_fnc <- function()
{
  experiment_config_check <- try(experiment_config_fnc(n_sim = n_sim, test_units = test_units, test_duration = test_duration, min_units = min_units, 
                                                       min_stress_levels = min_stress_levels, max_stress_levels = max_stress_levels, stress_distance = stress_distance, 
                                                       max_stress = max_stress, min_stress = min_stress, design_stress = design_stress,    
  ...
  if(inherits(experiment_config_check, "try-error"))
  {
    ...
    s1LB <- min_stress
    s1UB <- max_stress - (stress_distance*(min_stress_levels - 1))
    dLB <- stress_distance
    dUB <- (s1UB + stress_distance) - s1LB
    DELB <- c(min_stress_levels, s1LB, rep(dLB, times = (max_stress_levels - 1)), rep(-5, times = max_stress_levels))
    DEUB <- c(max_stress_levels, s1UB, rep(dUB, times = (max_stress_levels - 1)), rep(5, times = max_stress_levels))
    ...
    DEoptim_results <- DEoptim(objective_fnc, lower = DELB, upper = DEUB, control = DEoptim.control(reltol = DE_tol, steptol = DE_step, itermax = DE_iter),
                               n_sim = n_sim, test_units = test_units, max_stress = max_stress, stress_distance = stress_distance, min_units = min_units, 
                               p_cens_limit = p_cens_limit, filter_limit = filter_limit,
                               design_stress = design_stress, test_duration = test_duration, pquant = pquant, 
                               model_params = model_params, dp = dp, max_stress_levels = max_stress_levels,
                               s1LB = s1LB, s1UB = s1UB, dLB = dLB, dUB = dUB)
...
print_results_fnc <- function()
{
  ...
  }else
  {
    ...
    print_results <- results_fnc(x  = optimisation_results$optim$bestmem, test_units = test_units, min_units = min_units, 
                                 stress_distance = stress_distance, max_stress = max_stress, dp = dp, max_stress_levels = max_stress_levels,
                                 s1LB = s1LB, s1UB = s1UB, dLB = dLB, dUB = dUB, rmse = optimisation_results$optim$bestval)
...
stress_distance <- 10^(-1) #Minimum allowable distance between subsequent stress levels (must be > 0)
min_stress <- 0.1 #Minimum test stress
max_stress <- 0.9 #Maximum test stress
design_stress <- 0.05 #Nominal/operating stress
min_stress_levels <- 2 #Minimum number of stress levels (design points) (must be >= 2)
max_stress_levels <- 6 #Maximum number of stress levels (design points)
