############################################################################
### Test plan optimisation for Weibull power law AFT model (Specified N) ###
############################################################################
...
objective_function <- function(x, model_params = c(-7, -6, 2), test_units = 100,
...
ALT_optimise <- function(model_params = c(-7, -6, 2), test_units = 100,  
                         test_duration = 1*24*365, min_units = 1,
                         p_cens_limit = 0.8, filter_limit = 0.95,
                         pquant = 0.5, dp = 5, stress_distance = 10^(-5),
                         min_stress = 0.1, max_stress = 0.9, design_stress = 0.05,
                         stress_levels = 2, DE_tol = 0.05, n_sim = 1000, 
...
n_sim <- 1000 
#Number of required test plan stress levels (design points) (must be >= 2).
stress_levels <- 6
